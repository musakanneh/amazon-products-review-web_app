...
if __name__ == "__main__":
    ...
    def parse(path):
        """Create path for parsing the model

        Args:
            path(args): path to locate the model
    ...
    def getDF(path):
        ...
        Args:
            path(args): path to locate the data model
        """
        count_iteration = 0
        df = {}
    ...
    for name in category_names:
        ...
        for index, row in df.iterrows():
            treated_text.append(re.compile(
                f'([{string.punctuation}“”¨«»®´·º½¾¿¡§£₤‘’])').sub(r' \1 ', row['reviewText']))
